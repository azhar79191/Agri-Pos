...
import {
  getStockLogs, getStockAlerts, getStockLevels,
  getStockSummary, adjustStock, bulkAdjustStock,
} from "../api/stockApi";
...
export const useStockStore = create((set) => ({
  ...
  fetchLogs: async (params) => {
    ...
    try {
      ...
      set({
        logs: Array.isArray(data) ? data : data?.logs ?? data?.docs ?? [],
        loading: false,
      });
      return res.data;
    } catch (err) {
  ...
  adjust: async (data) => {
    ...
    // Sync updated stock level back into productStore so POS/Products pages update instantly
    if (result?.product?._id) {
      useProductStore.getState().editProduct(result.product._id, result.product);
    }
    return result;
  },
  ...
  bulkAdjust: async (data) => {
    ...
    // Re-fetch products to reflect all changes
