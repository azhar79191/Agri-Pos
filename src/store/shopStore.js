...
import {
  getMyShop, getShops, createShop, updateShop,
  deleteShop, getShopUsers, addUserToShop,
...
export const useShopStore = create((set) => ({
  ...
  updateShop: async (id, data) => {
    ...
    const shop = res.data.data?.shop ?? res.data.data;
    set((s) => ({
      shop: s.shop?._id === id ? shop : s.shop,
      shops: s.shops.map((sh) => (sh._id === id ? shop : sh)),
    }));
    return shop;
  },
  ...
  fetchShopUsers: async (id) => {
    const res = await getShopUsers(id);
    return res.data.data;
  },

  addUser: async (id, data) => {
    const res = await addUserToShop(id, data);
    return res.data.data;
  },
