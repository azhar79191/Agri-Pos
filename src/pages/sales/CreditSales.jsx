  ...
  return (
    <div className="space-y-6 animate-fade-up">
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-orange-500 to-amber-600 flex items-center justify-center shadow-glow-sm"><CreditCard className="w-5 h-5 text-white" /></div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { label: "Paid", value: rows.filter(s => s.status === "paid").length, color: "text-emerald-600 dark:text-emerald-400", bg: "bg-emerald-100 dark:bg-emerald-900/30", icon: Clock },
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1"><Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" /><input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search..." className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm text-slate-900 dark:text-white" /></div>
        <div className="flex gap-1 p-1 bg-slate-100 dark:bg-[#122b1c] rounded-xl">
          {[{ id: "all", l: "All" }, { id: "pending", l: "Pending" }, { id: "partial", l: "Partial" }, { id: "overdue", l: "Overdue" }, { id: "paid", l: "Paid" }].map(t => (
            <button key={t.id} onClick={() => setFilter(t.id)} className={`px-3 py-2 rounded-lg text-xs font-medium transition-all ${filter === t.id ? "bg-white dark:bg-[#0d1f14] text-emerald-600 shadow-sm" : "text-slate-500"}`}>{t.l}</button>
          ))}
        </div>
      </div>
