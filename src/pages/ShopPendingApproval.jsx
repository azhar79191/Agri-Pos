...
import {
  Clock, Mail, ArrowLeft, RefreshCw, CheckCircle,
  Zap, Crown, ShieldOff, AlertTriangle,
} from "lucide-react";
...
const ShopPendingApproval = ({ reason }) => {
  ...
  const isPending    = reason === "SHOP_PENDING_APPROVAL";

  // Clear auth and go to login — no logout API call, no toast, just redirect
  ...
  const description = isSuspended
    ? "Your shop has been suspended by the platform administrator. Please contact support to resolve this issue and restore access."
    : isPlanExpired
    ? "Your subscription plan has expired. Please contact the administrator to renew your plan and restore full access."
    : "Your shop has been registered successfully. Our team will review and activate your account shortly.";

  return (
    <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-6">

      {/* Status icon */}
      <div className={`w-20 h-20 rounded-2xl flex items-center justify-center mb-6 shadow-2xl border ${iconBg}`}>
        {icon}
      </div>

      <h1 className="text-3xl font-black text-white mb-3 text-center">{title}</h1>

      <p className="text-slate-400 text-center max-w-md mb-8 leading-relaxed">
        {description}
      </p>

      {/* Steps — only for pending approval */}
      {isPending && (
        <div className="w-full max-w-sm space-y-3 mb-8">
          {[
            ...
            >
              ...
              <span className={`text-sm font-medium ${done ? "text-emerald-300" : "text-slate-500"}`}>
                {label}
              </span>
              {done && (
                <span className="ml-auto text-[10px] font-bold text-emerald-400 bg-emerald-500/20 px-2 py-0.5 rounded-full">
                  Done
                </span>
              )}
            </div>
      ...
      {/* Plans preview — not shown when suspended */}
      {!isSuspended && (
        <div className="w-full max-w-2xl mb-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {PLANS.map((p) => (
              ...
              >
                {p.popular && (
                  <span className="text-[10px] font-bold text-blue-400 bg-blue-500/20 px-2 py-0.5 rounded-full mb-2 inline-block">
                    Popular
                  </span>
                )}
      ...
      <div className="flex items-center gap-2 text-sm text-slate-500 mb-6">
        ...
        <span>
          Contact:{" "}
          <a href="tel:+923071014374" className="text-blue-400 hover:underline">
           03071014374
          </a>
        </span>
      </div>
      ...
      <div className="flex gap-3">
        {/* Check Status — reload the page to re-probe */}
        <button
        ...
        {/* Back to Login — clears auth, no logout API, no confirm dialog */}
        <button
          onClick={handleBackToLogin}
          className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white transition-colors text-sm font-semibold"
        >
