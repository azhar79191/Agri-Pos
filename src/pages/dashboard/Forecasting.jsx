...
const Forecasting = () => {
  ...
  const STATS = [
    ...
    { l: "Next Month Forecast", v: formatCurrency(data.nextMonthForecast, settings.currency),  c: "text-blue-600 dark:text-blue-400" },
    { l: "Growth Trend",        v: `${data.avgGrowth >= 0 ? "+" : ""}${data.avgGrowth}%`,      c: "text-purple-600 dark:text-purple-400" },
  ];
  ...
  return (
    <div className="space-y-6 animate-fade-up">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Sales Forecasting</h1>
      ...
      {data.loading ? (
        <div className="flex items-center justify-center py-16 gap-2 text-slate-400">
          <Loader2 className="w-5 h-5 animate-spin" /><span className="text-sm">Loading forecast...</span>
        </div>
      ) : (
        <>
              ...
              <div className="h-72">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={data.combined}>
                    ...
                    <Tooltip contentStyle={{ borderRadius: "12px" }} formatter={(v, n) => [formatCurrency(v || 0, settings.currency), n === "actual" ? "Actual" : "Forecast"]} />
                    <Area type="monotone" dataKey="actual"   stroke="#10b981" fill="rgba(16,185,129,0.1)"  strokeWidth={2.5} connectNulls={false} />
                    <Area type="monotone" dataKey="forecast" stroke="#8b5cf6" fill="rgba(139,92,246,0.1)" strokeWidth={2.5} strokeDasharray="5 5" connectNulls={false} />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            ) : (
