...
import { SHOP_NAV } from "./shopConfig";

const ShopNav = ({ active, onChange, shopName, logoPreview }) => (
  <aside className="w-full lg:w-64 shrink-0">
    <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
      {/* Shop identity header */}
      <div className="px-4 py-4 border-b border-slate-100 dark:border-slate-800 bg-gradient-to-r from-blue-600 to-indigo-600">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-lg bg-white/20 backdrop-blur flex items-center justify-center overflow-hidden shrink-0">
            {logoPreview
              ? <img src={logoPreview} alt="logo" className="w-full h-full object-contain" />
              : <Store className="w-5 h-5 text-white" />}
          </div>
          <div className="min-w-0">
            <p className="text-sm font-bold text-white truncate">{shopName || "My Shop"}</p>
            <p className="text-xs text-blue-200">Shop Management</p>
          </div>
        </div>
      </div>
              ...
              return (
                <button key={item.id} onClick={() => onChange(item.id)}
                  ...
                  <div className="flex-1 min-w-0">
                    <p className={["text-xs font-semibold truncate", isActive ? "text-blue-700 dark:text-blue-400" : ""].join(" ")}>{item.label}</p>
                    <p className="text-[10px] text-slate-400 dark:text-slate-500 truncate">{item.desc}</p>
                  </div>
                  {isActive && <div className="w-1 h-5 rounded-full bg-blue-500 shrink-0" />}
                </button>
              );
            })}
          </div>
