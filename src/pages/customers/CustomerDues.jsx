...
import { Wallet, DollarSign, Users, Loader2, X, AlertTriangle, Phone } from "lucide-react";
...
const CustomerDues = () => {
  ...
  const isAdmin = true; // permission check done in hook via state
  ...
  return (
    <div className="space-y-5 animate-fade-up">
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-red-500 to-rose-600 flex items-center justify-center shadow-sm flex-shrink-0"><Wallet className="w-5 h-5 text-white" /></div>
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Customer Dues</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400">{total} customers with outstanding balances · Total: {formatCurrency(totalDue, settings.currency)}</p>
        </div>
      </div>
              ...
              return (
                <div key={d._id} className="flex items-center gap-4 px-5 py-4 hover:bg-slate-50/60 dark:hover:bg-slate-800/40 transition-colors">
                  ...
                  <div className="text-right flex-shrink-0">
                    <p className={`text-lg font-bold ${isHigh ? "text-red-600 dark:text-red-400" : "text-amber-600 dark:text-amber-400"}`}>{formatCurrency(due, settings.currency)}</p>
                    <p className="text-xs text-slate-400">outstanding</p>
                  </div>
                  <button onClick={() => { setDepositModal(d); setDepositAmount(""); }} className="flex-shrink-0 px-3 py-2 rounded-lg text-xs font-semibold border border-emerald-200 dark:border-emerald-800 text-emerald-700 dark:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 transition-colors ml-2">
                    Collect
                  </button>
                </div>
              );
