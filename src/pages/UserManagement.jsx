...
import { Plus, Edit3, Trash2, Shield, Eye, EyeOff, UserCheck, UserX, Settings, Crown, RefreshCw, Users } from "lucide-react";
...
const roleConfig = {
  ...
  cashier: { label: "Cashier",       cls: "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400", gradient: "from-emerald-500 to-teal-600" },
};
    ...
    <div className="space-y-6 animate-fade-up">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-violet-500 to-purple-600 flex items-center justify-center shadow-glow-sm">
        ...
        <div className="flex gap-2">
          <button onClick={() => fetchUsers()} className="flex items-center gap-2 px-3.5 py-2 rounded-xl border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 text-sm font-medium transition-colors">
            <RefreshCw className="w-4 h-4" />Refresh
          </button>
          {isAdmin && (
            <button onClick={() => { setFormData(emptyForm); setEditingUser(null); setShowAddUser(true); }}
              className="flex items-center gap-2 bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-white px-4 py-2 rounded-xl font-semibold text-sm transition-all shadow-glow-sm">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {users.map((user, i) => {
            ...
            return (
              <div key={user._id} className={`bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-700/50 shadow-premium hover:shadow-premium-lg hover:-translate-y-0.5 transition-all duration-200 overflow-hidden animate-fade-up stagger-${Math.min(i + 1, 4)}`}>
                <div className={`h-1 w-full bg-gradient-to-r ${rc.gradient}`} />
                <div className="p-5 space-y-4">
                  {/* User info */}
                  <div className="flex items-center gap-3">
                    <div className={`w-11 h-11 rounded-xl bg-gradient-to-br ${rc.gradient} flex items-center justify-center flex-shrink-0 shadow-sm`}>
                  ...
                  <div className="flex items-center justify-between px-1">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded-full font-semibold ${user.isActive !== false ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400" : "bg-red-100 text-red-700"}`}>
                  ...
                  {isAdmin && (
                    <div className="flex gap-1.5 pt-1 border-t border-slate-100 dark:border-slate-800">
                      ...
                      <button onClick={() => handleGrantAll(user._id)} className="p-1.5 rounded-lg bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400 hover:bg-emerald-100 dark:hover:bg-emerald-900/30 transition-colors" title="Grant All">
      ...
      {/* Add/Edit Modal */}
      {showAddUser && (
        <div className="fixed top-0 left-1/3 inset-0 z-[9999] flex h-[80vh] p-4" onClick={() => { setShowAddUser(false); setEditingUser(null); }}>
          <div className="absolute inset-0 w-fit modal-backdrop" />
          <div className="relative z-[10000] w-full max-w-md max-h-[90vh] flex flex-col overflow-hidden bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200/80 dark:border-slate-700/50" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4 overflow-y-auto flex-1">
              ...
              <div className="flex gap-3 pt-2">
                <button type="submit" disabled={submitting} className="flex-1 bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 disabled:opacity-50 text-white py-2.5 rounded-xl font-semibold text-sm transition-all">
                  {submitting ? "Saving..." : editingUser ? "Update User" : "Add User"}
                </button>
                <button type="button" onClick={() => { setShowAddUser(false); setEditingUser(null); setFormData(emptyForm); }} className="flex-1 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 py-2.5 rounded-xl font-semibold text-sm transition-colors">
                  Cancel
                </button>
              </div>
            </form>
      ...
      {showPermissions && selectedUser && (
        <div className="fixed inset-0 z-[9999] flex top-0 left-1/4  h-[80vh] p-4" onClick={() => setShowPermissions(false)}>
          <div className="absolute inset-0 w-fit modal-backdrop" />
          <div className="relative z-[10000] w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200/80 dark:border-slate-700/50" onClick={e => e.stopPropagation()}>
            <div className="px-6 py-5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
              ...
              <button onClick={() => setShowPermissions(false)} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl text-slate-500 text-xl leading-none transition-colors">×</button>
            </div>
            <div className="px-6 py-3 border-b border-slate-100 dark:border-slate-800 flex gap-2">
              <button onClick={() => handleGrantAll(selectedUser._id)} disabled={permSubmitting}
                className="flex items-center gap-2 px-4 py-2 bg-emerald-500 hover:bg-emerald-600 disabled:opacity-50 text-white rounded-xl text-xs font-semibold transition-colors">
                <UserCheck className="w-3.5 h-3.5" />Grant All
              </button>
              <button onClick={() => handleRevokeAll(selectedUser._id)} disabled={permSubmitting}
                className="flex items-center gap-2 px-4 py-2 bg-red-500 hover:bg-red-600 disabled:opacity-50 text-white rounded-xl text-xs font-semibold transition-colors">
                  ...
                  return (
                    <button key={perm.key} onClick={() => handlePermissionToggle(selectedUser._id, perm.key)}
                      className={`flex items-center justify-between p-3 rounded-xl border text-left transition-all ${
                        has ? "border-emerald-300 dark:border-emerald-700 bg-emerald-50 dark:bg-emerald-900/20"
