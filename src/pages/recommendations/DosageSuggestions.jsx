  ...
  return (
    <div className="space-y-6 animate-fade-up">
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-teal-500 to-cyan-600 flex items-center justify-center shadow-glow-sm"><Beaker className="w-5 h-5 text-white" /></div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-700/50 shadow-premium p-5">
        ...
        <div className="flex items-center gap-3"><label className="text-sm text-slate-600 dark:text-slate-400">Field Size (acres):</label><input type="number" value={fieldSize} onChange={e => setFieldSize(e.target.value)} min="0.1" step="0.5" className="w-24 px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-xl bg-white dark:bg-slate-800 text-sm text-slate-900 dark:text-white" /><span className="text-xs text-slate-400">Dosages below will be multiplied by field size</span></div>
      </div>

      <div className="space-y-4">
        {DOSAGE_DATA.map(item => (
          <div key={item.product} className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-700/50 shadow-premium p-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {item.crops.map(c => (
                <div key={c.crop} className="p-3 rounded-xl bg-slate-50 dark:bg-slate-800/60">
