  ...
  return (
    <div className="space-y-6 animate-fade-up">
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-emerald-500 to-green-600 flex items-center justify-center shadow-glow-sm"><DollarSign className="w-5 h-5 text-white" /></div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          { l: "Total Profit", v: formatCurrency(summary.totalProfit, settings.currency), c: "text-emerald-600 dark:text-emerald-400", bg: "bg-emerald-100 dark:bg-emerald-900/30" },
          ...
          <div key={l} className="stat-card-premium">
            <div className={`w-10 h-10 rounded-xl ${bg} flex items-center justify-center mb-3`}><DollarSign className={`w-5 h-5 ${c}`} /></div>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {rows.map((r, i) => (
                <tr key={i} className="hover:bg-slate-50/60 dark:hover:bg-slate-800/40 transition-colors">
                  ...
                  <td className="px-4 py-3.5"><span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${r.margin >= 25 ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400" : r.margin >= 20 ? "bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400" : "bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400"}`}>{r.margin}%</span></td>
