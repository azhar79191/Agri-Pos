...
const useInventoryReport = () => {
  ...
  useEffect(() => {
    getInventoryReport().then((res) => setData(res.data.data)).catch(() => {}).finally(() => setLoading(false));
  }, []);
  return { data, loading };
};
...
const InventoryReports = () => {
  ...
  const STATS = [
    ...
    { l: "Stock Value",    v: formatCurrency(summary.totalStockValue, settings.currency),c: "text-emerald-600 dark:text-emerald-400", bg: "bg-emerald-100 dark:bg-blue-900/20", icon: DollarSign },
  ...
  return (
    <div className="space-y-6 animate-fade-up">
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-teal-500 to-cyan-600 flex items-center justify-center shadow-sm"><Package className="w-5 h-5 text-white" /></div>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-lg border border-slate-200/80 dark:border-slate-700/50 shadow-premium p-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {Object.entries(categorySummary).map(([cat, d]) => (
                  <div key={cat} className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-700/50">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  {products.map((p) => (
                    <tr key={p._id} className="hover:bg-slate-50/60 dark:hover:bg-slate-800/40 transition-colors">
                      ...
                      <td className="px-4 py-3.5"><span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${p.status === "In Stock" ? "bg-emerald-100 text-emerald-700 dark:bg-blue-900/20 dark:text-emerald-400" : p.status === "Low Stock" ? "bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400" : "bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400"}`}>{p.status}</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
